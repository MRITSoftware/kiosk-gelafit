...
- **Tokens e Credenciais Expostas**: 
  - **Supabase Anon Key** hardcoded em 3 arquivos:
    - `lib/backend/supabase/supabase.dart`
    - `lib/backend/api_requests/api_calls.dart`
    - `lib/custom_code/actions/fila_relatorio_vendas_lote.dart`
  - **Firebase API Key** hardcoded em `lib/backend/firebase/firebase_config.dart`
  - **Nota:** Token do Mercado Pago (`tokenmp`) é obtido dinamicamente do banco ✅
  - **Recomendação**: Mover tokens para variáveis de ambiente (ver `TOKENS_EXPOSTOS_E_TESTES.md`)
...
### 🔴 Alta Prioridade
1. **Segurança**: Mover tokens Supabase e Firebase para variáveis de ambiente (ver `TOKENS_EXPOSTOS_E_TESTES.md`)
2. **Testes**: Implementar testes unitários para lógica crítica (ver `TOKENS_EXPOSTOS_E_TESTES.md` para plano completo)
