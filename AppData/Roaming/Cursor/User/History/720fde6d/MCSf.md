...
└── supabase-client.ts       ← Funções auxiliares para DB

types/
└── database.ts              ← Tipos TypeScript do banco (PT-BR)
...
### O banco já está configurado com nomes em PT-BR:

- ✅ **empresas** (ao invés de clients)
- ✅ **usuarios** (ao invés de users)
- ✅ **documentos** (ao invés de documents)
- ✅ **pastas** (ao invés de folders)
- ✅ **etiquetas** (ao invés de tags)
- ✅ **comentarios** (ao invés de comments)
- ✅ **notificacoes** (ao invés de notifications)
- ✅ E todas as outras tabelas em português!
...
export default function Dashboard() {
  const empresaId = 'seu-empresa-id' // Pegar do contexto/sessão
...
export async function POST(request: Request) {
  try {
    ...
    const documento = await criarDocumento({
      ...
      caminho_arquivo: `/uploads/${arquivo.name}`, // temporário
...
const novoComentario = await criarComentario({
  ...
  interno: false, // false = público, true = só admin vê
})
```
...
## 📦 5. Storage de Arquivos (Próximo Passo)

Para armazenar arquivos reais, você precisará:
...
## 🎯 6. Migrar Sistema Atual

### Passos para Migração:

1. **✅ FEITO**: Schema criado no Supabase
2. **✅ FEITO**: Tipos TypeScript gerados
3. **✅ FEITO**: Funções auxiliares criadas
4. **✅ FEITO**: Hooks React criados

### Próximos Passos:

5. **Substituir `lib/database.ts`** pelas funções de `lib/supabase-client.ts`
6. **Atualizar componentes** para usar os novos hooks
7. **Migrar autenticação** para Supabase Auth
...
Tente buscar dados com diferentes usuários para garantir que as políticas RLS estão funcionando.

---

## 📚 8. Referências

### Funções Disponíveis em `lib/supabase-client.ts`:
...
### Hooks Disponíveis em `hooks/useSupabase.ts`:
...
**Solução:** Verifique se as políticas RLS estão ativas e se o usuário está autenticado

### Erro: "relation X does not exist"
**Solução:** Execute o schema SQL no Supabase (supabase-schema.sql)

---

**Agora você está pronto para usar o Supabase com nomes em PT-BR! 🚀**
