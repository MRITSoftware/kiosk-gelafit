...
### Opção 1: Direto na tabela `displays` (Recomendado):
```sql
UPDATE displays 
SET tipo = 'Promo' 
WHERE codigo_unico = 'SUA_TELA_AQUI';
```

### Opção 2: Para conteúdo único (tabela `conteudos`):
```sql
INSERT INTO conteudos (codigoAnuncio, tipo, url, duration) 
VALUES ('PROMO001', 'Promo', 'https://exemplo.com/promo.jpg', 15000);
```

### Opção 3: Para itens de playlist (tabela `playlist_itens`):
```sql
